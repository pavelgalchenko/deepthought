...
double SpinGainCostFunction(double p[2], double CostParm[2])
{
   ...
   Coef[4] = (p[0] - K1 * p[1]) * (p[0] - K1 * p[1]) * Om * Om +
             K1 * K1 * Om * Om * Om * Om;
...
vec3_t PointGimbalToTarget(long Seq, mat3x3_t CGiBi, mat3x3_t CBoGo, vec3_t tvi,
                           vec3_t bvo)
{
   vec3_t *a1, *a2;
   mat3x3_t Axis = MAT3X3_EYE;
   vec3_t TargVec, BoreVec, a3;
...
vec3_t CollisionAvoidanceLaw(vec3_t x, vec3_t v, vec3_t xg, vec3_t xa,
...
/**********************************************************************/
vec3_t VectorRampCoastGlide(vec3_t Xvec, vec3_t Vvec, double w0, double amax,
                            double vmax)
{
   vec3_t Axis, Xaxis, Yaxis;
...
void AllocKalmanFilterMeasurement(struct KFMeasType *M, long Nx, long Ny)
{
   ...
   for (i = 0; i < Nx; i++)
      M->L[i] = (double *)calloc(Ny, sizeof(double));

   M->HP = (double **)calloc(Ny, sizeof(double *));
   for (i = 0; i < Ny; i++)
      M->HP[i] = (double *)calloc(Nx, sizeof(double));

   M->Hx = (double *)calloc(Ny, sizeof(double));

   M->HPHtRv = (double **)calloc(Ny, sizeof(double *));
   for (i = 0; i < Ny; i++)
      M->HPHtRv[i] = (double *)calloc(Ny, sizeof(double));
   ...
   if (M->Ny == 1) {
      ...
      Det = M->HPHtRv[0][0] * (M->HPHtRv[1][1] * M->HPHtRv[2][2] -
            ...
            M->HPHtRv[0][1] * (M->HPHtRv[1][2] * M->HPHtRv[2][0] -
                               M->HPHtRv[2][2] * M->HPHtRv[1][0]) +
            M->HPHtRv[0][2] * (M->HPHtRv[1][0] * M->HPHtRv[2][1] -
...
double CMGLaw4x1DOF(vec3_t Tcmd, vec3_t Axis[4], vec3_t Gim[4], quat_t h,
                    quat_t *AngRateCmd)
{
   double eps0  = 0.1;
   double lam0  = 0.01;
   ...
   for (i = 0; i < 3; i++) {
      for (j = 0; j < 3; j++) {
         AAt[i][j] = A[i][0] * A[j][0] + A[i][1] * A[j][1] + A[i][2] * A[j][2] +
                     A[i][3] * A[j][3];
      }
   }
   /* Gain measures proximity to singularities */
   Gain =
       AAt[0][0] * AAt[1][1] * AAt[2][2] + AAt[0][1] * AAt[1][2] * AAt[2][0] +
   ...
   for (i = 0; i < 3; i++) {
      for (j = 0; j < 4; j++) {
         AW[i][j] = A[i][0] * W[0][j] + A[i][1] * W[1][j] + A[i][2] * W[2][j] +
                    A[i][3] * W[3][j];
      }
   }
   /* Den = AWAt + V*/
