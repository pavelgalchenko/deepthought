      ...
      if (State == GLUT_DOWN) {
         if (x >= PovWidget.xmin && x <= PovWidget.xmax &&
            ...
            for (i = 0; i < PovWidget.Nspot; i++) {
               if (x >= PovWidget.Spot[i].xmin && x <= PovWidget.Spot[i].xmax &&
                   y >= PovWidget.Spot[i].ymin && y <= PovWidget.Spot[i].ymax) {
                  Pick = i;
               }
            }
            if (Pick < 3) {
...
void CamMouseActiveMotionHandler(int x, int y)
{
   if (POV.Mode == TRACK_HOST) {
      POV.w.y = MouseScaleFactor * (double)(x - MouseClickX);
      POV.w.x = MouseScaleFactor * (double)(y - MouseClickY);
      POV.w.z = 0.0;
   }
}
/**********************************************************************/
         ...
         if (x >= OrreryWidget.xmin && x <= OrreryWidget.xmax &&
            ...
            switch (Pick) {
               ...
               case 0: /* Zoom In */
                  ...
                  O->Radius =
                      ((double)OrreryWidth) / (2.0 * 80.0) * O->Scale[O->Zoom];
                  break;
               case 1: /* Zoom Out */
         ...
         /* Center on World or Lagrange Point */
            ...
            for (Iw = 0; Iw < NWORLD && !Done; Iw++) {
               if (World[Iw].Exists) {
                  ...
                  xw = 0.5 * (1.0 + rw.x / O->Radius) * OrreryWidth;
                  yw = 0.5 *
                       (1.0 - rw.y / (O->Radius * OrreryHeight / OrreryWidth)) *
                       OrreryHeight;
                  dx = ((double)x) - xw;
                  dy = ((double)y) - yw;
                  d  = sqrt(dx * dx + dy * dy);
            ...
            for (Is = 0; Is < 3 && !Done; Is++) {
               if (LagSys[Is].Exists) {
                  ...
                  for (Ip = 0; Ip < 5; Ip++) {
                     ...
                     yw = 0.5 *
                          (1.0 -
                           rw.y / (O->Radius * OrreryHeight / OrreryWidth)) *
                          OrreryHeight;
                     dx = ((double)x) - xw;
                     dy = ((double)y) - yw;
                     d  = sqrt(dx * dx + dy * dy);
...
void SphereReshapeHandler(int width, int height __attribute__((unused)))
{
   ...
   ymin =
       -90.0 - 16 * (NumSphereWindowMenuLines + 2) * 360.0 / SphereWindowWidth;
...
void InitOrreryWindow(void)
{
   ...
   glMatrixMode(GL_PROJECTION);
   glLoadIdentity();
   glOrtho(-O->Radius, O->Radius, -O->Radius, O->Radius, -60.0 * AU2m,
           60.0 * AU2m);
   glMatrixMode(GL_MODELVIEW);
...
long GuiCmdInterpreter(char CmdLine[512], double *CmdTime)
{
   ...
   if (sscanf(CmdLine, "%lf POV.Host.RefOrb %ld", CmdTime, &POV.Host.RefOrb) ==
       2) {
      NewCmdProcessed = TRUE;
   ...
   if (sscanf(CmdLine, "%lf FOV[%ld].NearExists =  %s", CmdTime, &Ifov,
              response) == 3) {
      NewCmdProcessed      = TRUE;
      FOV[Ifov].NearExists = DecodeString(response);
   }

   if (sscanf(CmdLine, "%lf FOV[%ld].FarExists =  %s", CmdTime, &Ifov,
              response) == 3) {
      NewCmdProcessed     = TRUE;
   ...
   if (sscanf(CmdLine, "%lf BannerColor = [%lf %lf %lf %lf]", CmdTime, &Val1,
              &Val2, &Val3, &Val4) == 5) {
      NewCmdProcessed = TRUE;
      BannerColor[0]  = Val1;
      BannerColor[1]  = Val2;
   ...
   if (sscanf(CmdLine, "%lf POV CmdAngle = [%lf %lf %lf] deg", CmdTime, &Ang1,
              &Ang2, &Ang3) == 4) {
      NewCmdProcessed = TRUE;
      POV.CmdAngle.x  = Ang1 * D2R;
      POV.CmdAngle.y  = Ang2 * D2R;
      POV.CmdAngle.z  = Ang3 * D2R;
   }
