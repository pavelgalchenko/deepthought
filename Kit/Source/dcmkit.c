...
void Q2W(double q[4], double qdot[4], double w[3])
{
   ...
   w[1] = 2.0 *
          (-qdot[0] * q[2] + qdot[1] * q[3] + qdot[2] * q[0] - qdot[3] * q[1]);

   w[2] = 2.0 *
          (qdot[0] * q[1] - qdot[1] * q[0] + qdot[2] * q[3] - qdot[3] * q[2]);
}
/**********************************************************************/
                   ...
                   double Ds[3], double Dds[3])
{
   ...
   if (IsSpherical) {
      ...
      for (i = 0; i < 3; i++)
         Gs[i] =
             Gamma[i][0] * sig[0] + Gamma[i][1] * sig[1] + Gamma[i][2] * sig[2];
   }
