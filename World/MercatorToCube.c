   ...
   char pathname[40],char filename[40])
{
      ...
      long face,i,j;
      long idx = 0;
      char Face[6][20] = {"ColPX.ppm","ColPY.ppm","ColPZ.ppm",
                          "ColMX.ppm","ColMY.ppm","ColMZ.ppm"};
...
int main(int argc, char **argv)
{
      ...
      if (argc != 4) {
         printf("Need World File Name (w/o extension), width, and heigth as arguments.\n");
         exit(1);
      }
      ...
      for(F=0;F<6;F++) {
         for(I=0;I<RES;I++) {
            for(J=0;J<RES;J++) {
               ...
               lat = asin(p[2]);
               lng = atan2(p[1],p[0]);
               i = (long) ((Heigth-1)*(-lat+HalfPi)/Pi+0.5);
               j = (long) ((Width-1)*(lng+Pi)/TwoPi+0.5);
               for(k=0;k<3;k++) {
                  Cube[RES*RES*3*F+RES*3*I+3*J+k] = Tex[Width*3*i+3*j+k];
               }
               /* Gloss */
               Red = (double) Tex[Width*3*i+3*j+0];
               Grn = (double) Tex[Width*3*i+3*j+1];
               Blu = (double) Tex[Width*3*i+3*j+2];
               if (Blu > Red+Grn) {
