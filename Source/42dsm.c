...
void ThrProcessingMinPower(struct SCType *S) {
   ...
   for(i=0; i<AC->Nthr; i++) {
      distDotCmd = 0.0;
      for (j=0; j<6; j++) distDotCmd += AC->Thr[i].DistVec[j]*cmdVec[j];
...
void InitThrDistVecs(struct AcType *AC, int DOF, long controllerState) {
   ...
   for (i=0;i<AC->Nthr; i++) {
      for (j=0;j<6;j++) AC->Thr[i].DistVec[j] = 0.0;
   }
   ...
   for (i=0; i<AC->Nthr; i++) {
      for (j=0; j<3; j++) {
         if (DOF==3){
            ...
            } else if (controllerState==ATT_STATE || controllerState==DMP_STATE) {
   ...
   for (i=0; i<AC->Nthr; i++) {
      for (j=0; j<DOF; j++) {
         A[j][i] *= AC->Thr[i].Fmax; // Without this, errors would arise if thrusters have different max thrusts
      }
   }
   ...
   for (i=0; i<AC->Nthr; i++) {
      if (DOF == 3) {
         // Unused entries of DistVec will be zero, so won't cause issues with eventual dot product
         for (j=0; j<DOF; j++) {
            if (controllerState==TRN_STATE) {
...
//----------------------------------- GAINS -----------------------------------
long GetGains(struct SCType *S, const char GainCmdName[255], long controllerState, FILE *InpDsmFilePtr) {
   ...
   while (fgets(DsmCmdLine, 255, InpDsmFilePtr)) {
      if (sscanf(DsmCmdLine, GainCmd, &GainMode) == 1) {
         if (!strcmp(GainMode, "PID")) {
            strcat(GainCmd, " Kp %lf %lf %lf Kr %lf %lf %lf Ki %lf %lf %lf Ki_Limit %lf %lf %lf");
            if (sscanf(DsmCmdLine, GainCmd, GainMode, &kp[0], &kp[1], &kp[2], &kr[0], &kr[1], &kr[2], &ki[0], &ki[1], &ki[2], &limit_vec[0], &limit_vec[1], &limit_vec[2]) == 13)
               if (controller == PID_CNTRL)
                  GainsProcessed = TRUE;
         }
         else if (!strcmp(GainMode, "PID_WN")) {
            strcat(GainCmd, " %lf %lf %lf %lf");
            if (sscanf(DsmCmdLine, GainCmd, &GainMode, &omega, &zeta, &alpha, &limit) == 5) {
               for (i = 0; i<3; i++) {
            ...
            switch (controller) {
               case LYA_2BODY_CNTRL:
                  ...
                  if (sscanf(DsmCmdLine, GainCmd, GainMode, &omega, &zeta) == 3) {
                     for (i=0; i<3; i++) {
            ...
            if (controllerState != DMP_STATE) {
               printf("%s gain sets can only be used for momentum dumping. Exiting...",GainMode);
               exit(EXIT_FAILURE);
            }
            strcat(GainCmd, " %lf %lf %lf");
            if (sscanf(DsmCmdLine, GainCmd, GainMode, &kp[0], &kp[1], &kp[2]) == 4)
               GainsProcessed = TRUE;
         }
         break;
      }
   ...
   if (GainsProcessed == TRUE){  // Set GainsProcessed flag for relevant controller
      if (controllerState == TRN_STATE){
...
//----------------------------------- LIMITS -----------------------------------
long GetLimits(struct SCType *S, const char LimitCmdName[255], long controllerState, FILE *InpDsmFilePtr) {
   ...
   rewind(InpDsmFilePtr);
   while (fgets(DsmCmdLine, 255, InpDsmFilePtr)) {
      if (sscanf(DsmCmdLine, LimitCmd, &fMax[0], &fMax[1], &fMax[2], &vMax[0], &vMax[1], &vMax[2])==6) {
...
//----------------------------------- CONTROLLER -----------------------------------
long GetController(struct SCType *S, const char CtrlCmdName[255], long controllerState, FILE *InpDsmFilePtr) {
   ...
   rewind(InpDsmFilePtr);
   while (fgets(DsmCmdLine, 255, InpDsmFilePtr)) {
      if (sscanf(DsmCmdLine, CtrlCmd, &ControllerCmdMode, &CtrlGainCmd, &CtrlLimitCmd) == 3) {
         if (!strcmp(ControllerCmdMode, "PID_CNTRL")) controller = PID_CNTRL;
         else if (!strcmp(ControllerCmdMode, "LYA_ATT_CNTRL")) controller = LYA_ATT_CNTRL;
         else if (!strcmp(ControllerCmdMode, "LYA_2BODY_CNTRL")) controller = LYA_2BODY_CNTRL;
         else if (!strcmp(ControllerCmdMode, "H_DUMP_CNTRL")) controller = H_DUMP_CNTRL;
         else {
            printf("%s is an invalid control type. Exiting...\n",ControllerCmdMode);
            exit(EXIT_FAILURE);
         }
         // There should be a nicer way to handle this that doesn't require hardcoding for things...
         ...
         if (controller == H_DUMP_CNTRL && controllerState != DMP_STATE) {
            printf("%s\n","Can only use H_DUMP_CNTRL for momentum dumping control. Exiting...");
            exit(EXIT_FAILURE);
         }
         if (controller == LYA_2BODY_CNTRL && controllerState != TRN_STATE) {
            printf("%s\n", "Can only use LYA_2BODY_CNTRL for translation control. Exiting...");
            exit(EXIT_FAILURE);
         }
         CntrlProcessed = TRUE;
         break;
...
//----------------------------------- ACTUATORS -----------------------------------
long GetActuators(struct SCType *S, const char ActuatorCmdName[255], long controllerState, FILE *InpDsmFilePtr) {
   ...
   while (fgets(DsmCmdLine, 255, InpDsmFilePtr)) {
      if (sscanf(DsmCmdLine, ActuatorCmd, &ActuatorName) == 1) {
         if (sscanf(ActuatorName, "WHL_[%d]", &H_DumpCmdMode) == 1) {
            ...
            strcpy(Cmd->dmp_actuator,""); // Null it out if no dumping to avoid other errors
            if (controllerState == ATT_STATE) Cmd->H_DumpActive = FALSE;
         }
...
//------------------------- TRANSLATIONAL CMD ----------------------------------
long GetTranslationCmd(struct SCType *S, char TranslationCmd[255], double DsmCmdTime, FILE *InpDsmFilePtr) {
   char DsmCmdLine[255] = "";
   ...
   if (!strcmp(TranslationCmd, "NO_CHANGE")) {
      ...
      rewind(InpDsmFilePtr);
      while(fgets(DsmCmdLine, 255, InpDsmFilePtr)) { // Start Looping through file until reach EOF
         // Extract Translation Command
         if (sscanf(DsmCmdLine, TranslationCmd, &Cmd->Pos[0], &Cmd->Pos[1], &Cmd->Pos[2], &Cmd->RefOrigin, &Cmd->RefFrame, &ControllerCmd, &ActuatorCmd) == 7) {
            TranslationCmdProcessed = TRUE;
      ...
      while(fgets(DsmCmdLine, 255, InpDsmFilePtr)) { // Start Looping through file until reach EOF
         ...
         if (sscanf(DsmCmdLine, TranslationCmd, &Cmd->DeltaV[0], &Cmd->DeltaV[1], &Cmd->DeltaV[2], &Cmd->RefFrame, &ManeuverCmdMode, &Cmd->BurnTime, &CtrlLimitCmd, &ActuatorCmd) == 8) {
            TranslationCmdProcessed = TRUE;
            Cmd->BurnStopTime = DsmCmdTime + Cmd->BurnTime;
            if (!strcmp(ManeuverCmdMode, "CONSTANT")) Cmd->ManeuverMode = CONSTANT;
            else if (!strcmp(ManeuverCmdMode, "SMOOTHED")) Cmd->ManeuverMode = SMOOTHED;
            else {
               printf("%s is an invalid maneuver mode for %s. Exiting...",ManeuverCmdMode, TranslationCmdName);
               exit(EXIT_FAILURE);
            }
            break;
         }
   ...
   if (TranslationCmdProcessed == TRUE && Cmd->TranslationCtrlActive==TRUE) {
      if (Cmd->ManeuverMode == INACTIVE) {
         if (GetController(S,ControllerCmd,TRN_STATE, InpDsmFilePtr) == FALSE) {
...
//-----------------------ATTITUDE CMD ---------------------------------------
long GetAttitudeCmd(struct SCType *S, char AttitudeCmd[255], FILE *InpDsmFilePtr) {
   ...
   long AttitudeCmdProcessed = FALSE, AttPriCmdProcessed = FALSE, AttSecCmdProcessed = FALSE;
   long Isc_trgt, PriTrgTypeFlag = 0, SecTrgTypeFlag = 0, GroundStationNum, target_num;
   int state = ATT_STATE;
   ...
   else if (sscanf(AttitudeCmd, "AttitudeCmd_PV[%d]_SV[%d]", &AttPriCmdMode, &AttSecCmdMode) == 2 ||
            sscanf(AttitudeCmd, "AttitudeCmd_SV[%d]_PV[%d]", &AttSecCmdMode, &AttPriCmdMode) == 2) { // Decode Cmd into Method and Mode for setting params
      strcpy(AttCmdMethod, "AttitudeCmd");
      Cmd->AttitudeCtrlActive = TRUE;
   }
   else if (sscanf(AttitudeCmd, "AttitudeCmd_PV[%d]", &AttCmdMode) == 1 ) { // Decode Cmd into Method and Mode for setting params
   ...
   if (!strcmp(AttCmdMethod,"AttitudeCmd")) {
      ...
      if (!strcmp(PriTrgType,"BODY") || !strcmp(PriTrgType,"SC")) {
         ...
         rewind(InpDsmFilePtr);
         while(fgets(DsmCmdLine, 255, InpDsmFilePtr)) { // Start Looping through file until reach EOF
            if (sscanf(DsmCmdLine, PriTargetCmd, Junk, &PV->cmd_axis[0], &PV->cmd_axis[1], &PV->cmd_axis[2], &Target, &ControllerCmd, &ActuatorCmd) == 7) {
               if (!strcmp(PriTrgType, "BODY")) {
                  PV->TrgType = TARGET_WORLD;
                  strcpy(GroundStationCmd, "GroundStation_[%ld]");
                  if (sscanf(Target, GroundStationCmd, &GroundStationNum) == 1) {
                     PV->TrgWorld = GroundStation[GroundStationNum].World;
                     for(int i=0;i<3;i++) PV->W[i] = GroundStation[GroundStationNum].PosW[i];
                  }
                  else {
                     PV->TrgWorld = DecodeString(Target);
                     for(int i=0;i<3;i++) PV->W[i] = 0.0;
                  }
               }
               else if (!strcmp(PriTrgType, "SC")) {
                  if (sscanf(Target, "SC[%ld].B[%ld]", &Isc_trgt, &target_num) == 2){ // Decode Current SC ID Number
                     if (Isc_trgt >= Nsc) {
                        printf("This mission only has %ld spacecraft, but spacecraft %ld was attempted to be set as the primary target vector. Exiting...\n", Nsc, Isc_trgt);
                        exit(EXIT_FAILURE);
                     }
                     if (target_num >= SC[Isc_trgt].Nb){
                        printf("Spacecraft %ld only has %ld bodies, but the primary target was attempted to be set as body %ld. Exiting...\n", Isc_trgt, SC[Isc_trgt].Nb, target_num);
                        exit(EXIT_FAILURE);
                     }
         ...
         if (AttPriCmdProcessed == FALSE) {
            printf("%s has improper format for SC or BODY targeting. Exiting...\n", PriTargetCmdName);
            exit(EXIT_FAILURE);
         }
      }
         ...
         rewind(InpDsmFilePtr);
         while(fgets(DsmCmdLine, 255, InpDsmFilePtr)) { // Start Looping through file until reach EOF
            if (sscanf(DsmCmdLine, PriTargetCmd, Junk, &PV->cmd_axis[0], &PV->cmd_axis[1], &PV->cmd_axis[2], &Cmd->PriAttRefFrame, &PV->cmd_vec[0], &PV->cmd_vec[1], &PV->cmd_vec[2], &ControllerCmd, &ActuatorCmd) == 10) {
               AttPriCmdProcessed = TRUE;
               break;
            }
         }
      ...
      if (!strcmp(SecTrgType,"BODY") || !strcmp(SecTrgType,"SC")) {
         ...
         rewind(InpDsmFilePtr);
         while(fgets(DsmCmdLine, 255, InpDsmFilePtr)) { // Start Looping through file until reach EOF
            if (fscanf(InpDsmFilePtr, SecTargetCmd, Junk, &SV->cmd_axis[0], &SV->cmd_axis[1], &SV->cmd_axis[2], &Target) == 5) {
               if (!strcmp(SecTrgType, "BODY")) {
                  SV->TrgType = TARGET_WORLD;
                  strcpy(GroundStationCmd, "GroundStation_[%ld]");
                  if (sscanf(Target, GroundStationCmd, &GroundStationNum) == 1) {
                     SV->TrgWorld = GroundStation[GroundStationNum].World;
                     for(int i=0;i<3;i++) SV->W[i] = GroundStation[GroundStationNum].PosW[i];
                  }
                  else {
                     SV->TrgWorld = DecodeString(Target);
                     for(int i=0;i<3;i++) SV->W[i] = 0.0;
                  }
               }
               else if (!strcmp(SecTrgType, "SC")) {
                  if (sscanf(Target, "SC[%ld].B[%ld]", &Isc_trgt, &target_num) == 2){ // Decode Current SC ID Number
                     if (Isc_trgt >= Nsc) {
                        printf("This mission only has %ld spacecraft, but spacecraft %ld was attempted to be set as the secondary target vector. Exiting...\n", Nsc, Isc_trgt);
                        exit(EXIT_FAILURE);
                     }
                     if (target_num >= SC[Isc_trgt].Nb){
                        printf("Spacecraft %ld only has %ld bodies, but the secondary target was attempted to be set as body %ld. Exiting...\n", Isc_trgt, SC[Isc_trgt].Nb, target_num);
                        exit(EXIT_FAILURE);
                     }
         ...
         rewind(InpDsmFilePtr);
         while(fgets(DsmCmdLine, 255, InpDsmFilePtr)) { // Start Looping through file until reach EOF
            if (fscanf(InpDsmFilePtr, SecTargetCmd, Junk, &SV->cmd_axis[0], &SV->cmd_axis[1], &SV->cmd_axis[2], &Cmd->SecAttRefFrame, &SV->cmd_vec[0], &SV->cmd_vec[1], &SV->cmd_vec[2]) == 8) {
               AttSecCmdProcessed = TRUE;
               break;
            }
         }
      ...
      rewind(InpDsmFilePtr);
      while(fgets(DsmCmdLine, 255, InpDsmFilePtr)) { // Start Looping through file until reach EOF
         if (sscanf(DsmCmdLine, AttitudeCmd, &Cmd->q[0], &Cmd->q[1], &Cmd->q[2], &Cmd->q[3], &Cmd->AttRefFrame, &ControllerCmd, &ActuatorCmd) == 7) {
      ...
      rewind(InpDsmFilePtr);
      while(fgets(DsmCmdLine, 255, InpDsmFilePtr)) { // Start Looping through file until reach EOF
         if (sscanf(DsmCmdLine, AttitudeCmd, &Cmd->AttRefScID, &ControllerCmd, &ActuatorCmd) == 3) {
      ...
      rewind(InpDsmFilePtr);
      while(fgets(DsmCmdLine, 255, InpDsmFilePtr)) { // Start Looping through file until reach EOF
         if (sscanf(DsmCmdLine, AttitudeCmd, &ControllerCmd, &ActuatorCmd) == 2) {
      ...
      rewind(InpDsmFilePtr);
      while (fgets(DsmCmdLine, 255, InpDsmFilePtr)) {
         if (sscanf(DsmCmdLine, AttitudeCmd, &OnOff, &Cmd->H_DumpLims[0], &Cmd->H_DumpLims[1], &ControllerCmd, &ActuatorCmd) == 5) {
            state = DMP_STATE;
            if (!strcmp(OnOff,"ON"))
            ...
            if (Cmd->H_DumpLims[1]<Cmd->H_DumpLims[0]) {
               printf("Maximum momentum dump limit must be more than the minimum for %s. Exiting...\n",AttitudeCmdName);
               exit(EXIT_FAILURE);
            }
         }
      }
   ...
   // Extract Controllers, Gains, Limits, & Actuators Corresponding to the Ctrl mode commanded
...
long GetActuatorCmd(struct SCType *S, char ActuatorCmd[255], FILE *InpDsmFilePtr) {
   ...
   while(fgets(DsmCmdLine, DsmCmdLinelength, InpDsmFilePtr)) { // Start Looping through file until reach EOF
      ...
      if (sscanf(DsmCmdLine, ActuatorCmd, &NumCommands, &SubCommands) == 2) {
         ...
         if (i < NumCommands) {
            printf("Declared NUM_CMD[%d], but cound only find [%d] commands. Exiting...\n", NumCommands, i);
            exit(EXIT_FAILURE);
         }
         for (i=0; i<NumCommands; i++)
         {
            if (sscanf(each_command[i], "WHL_[%d]_[%lf]", &Cmd->ActInds[i], &Cmd->ActDuties[i]) == 2) Cmd->ActTypes[i] = WHL_TYPE;
            else if (sscanf(each_command[i], "THR_[%d]_[%lf]", &Cmd->ActInds[i], &Cmd->ActDuties[i]) == 2) Cmd->ActTypes[i] = THR_TYPE;
            else if (sscanf(each_command[i], "MTB_[%d]_[%lf]", &Cmd->ActInds[i], &Cmd->ActDuties[i]) == 2) Cmd->ActTypes[i] = MTB_TYPE;
            else {
               printf("The command %s is not a valid command. Exiting...\n",each_command[i]);
               exit(EXIT_FAILURE);
            }
            if (Cmd->ActTypes[i] == WHL_TYPE && Cmd->ActInds[i] > AC->Nwhl){
               printf("SC[%ld] only has %ld wheels, but an actuator command was sent to wheel %d. Exiting...\n", AC->ID, AC->Nwhl, Cmd->ActInds[i]);
               exit(EXIT_FAILURE);
            }
            if (Cmd->ActTypes[i] == THR_TYPE && Cmd->ActInds[i] > AC->Nthr){
               printf("SC[%ld] only has %ld thrusters, but an actuator command was sent to thruster %d. Exiting...\n", AC->ID, AC->Nthr, Cmd->ActInds[i]);
               exit(EXIT_FAILURE);
            }
            if (Cmd->ActTypes[i] == MTB_TYPE && Cmd->ActInds[i] > AC->Nmtb){
               printf("SC[%ld] only has %ld MTBs, but an actuator command was sent to MTB %d. Exiting...\n", AC->ID, AC->Nmtb, Cmd->ActInds[i]);
               exit(EXIT_FAILURE);
            }
         }
         ...
         if (NumCommands == 0) {
            ...
            printf("%d sub-commands were specified, but no subcommands were found. Exiting...\n", NumCommands);
            exit(EXIT_FAILURE);
         }
      }
   }
...
void DsmCmdInterpreterMrk1(struct SCType *S, FILE *InpDsmFilePtr) {
   ...
   DSM->CmdCnt = 0;
   for (i = 0; i < 100; i++) DSM->CmdTime_f[i] = 0.0;

   rewind(InpDsmFilePtr);
   while(fgets(DSM_FileLine, DSM_CmdLineLength, InpDsmFilePtr)) { // Start Looping through file until reach EOF
      // Read Spacecraft Command------------------------------------------------
      strcpy(DSM_CmdLine, "DSM_Cmd %s %s %lf");
      if (sscanf(DSM_FileLine, DSM_CmdLine, &ScID, Junk, &DSM_CmdTime) == 3) {
         if (sscanf(ScID, "SC[%ld]", &Isc) != 1) {  // Decode Current SC ID Number
      ...
      else if (!strncmp(DSM_FileLine," ",1) || !strncmp(DSM_FileLine,"//",2) || !strncmp(DSM_FileLine,"#",1) || !strncmp(DSM_FileLine,"<",1) ||
         !strncmp(DSM_FileLine,"\n",1) || !strncmp(DSM_FileLine,"\r",1) || !strncmp(DSM_FileLine,"%",1)) {
      }
...
void DsmCmdInterpreterMrk2(struct SCType *S, FILE *InpDsmFilePtr) {
   ...
   rewind(InpDsmFilePtr);
   while(fgets(DSM_FileLine, DsmCmdLinelength, InpDsmFilePtr)) { // Start Looping through file until reach EOF
      // Read Spacecraft Command------------------------------------------------
      strcpy(DSM_CmdLine, "DSM_Cmd %s %s %lf");
      if (sscanf(DSM_FileLine, DSM_CmdLine, &ScID, Junk, &DSM_CmdTime) == 3) {
         if (sscanf(ScID, "SC[%ld]", &Isc) != 1) {  // Decode Current SC ID Number
         ...
         if (S->ID == Isc && DSM->CmdNextTime == DSM_CmdTime) {
            ...
            if (sscanf(DSM_FileLine, DSM_CmdLine, Junk, Junk, Junk, &NumCommands, &SubCommands) == 5) {
               i = 0;
               token = strtok (SubCommands, " ");
               while (token != NULL) {
                  each_command[i++] = token;
            ...
            // Set Attitude and Translation as NO_CHANGE - will get overwritten if there is a change
            ...
            for (i=0; i<NumCommands; i++) {
               ...
               if (!strncmp(curCommand, "DetumbleCmd", 11) || !strncmp(curCommand, "AttitudeCmd", 11) || !strncmp(curCommand, "QuaternionCmd", 13) || !strncmp(curCommand, "MirrorCmd", 9) || !strncmp(curCommand, "WhlHManageCmd", 13) || !strcmp(curCommand, "PASSIVE_ATT") ) {
                  strcpy(AttitudeCmd, curCommand);
                  if (GetAttitudeCmd(S, AttitudeCmd, InpDsmFilePtr) == FALSE){
                     printf("%s does not match any valid attitude command methods found in Inp_DSM.txt. Exiting...\n", AttitudeCmd);
                     exit(EXIT_FAILURE);
                  }
               }
               // Parse Translation Commands
               else if (!strncmp(curCommand, "TranslationCmd", 14) || !strncmp(curCommand, "ManeuverCmd", 11) || !strcmp(curCommand, "PASSIVE_TRN") ){
                  strcpy(TranslationCmd, each_command[i]);
                  if (GetTranslationCmd(S, TranslationCmd, DSM_CmdTime, InpDsmFilePtr) == FALSE){
                     printf("%s does not match any valid translational command methods found in Inp_DSM.txt. Exiting...\n", TranslationCmd);
                     exit(EXIT_FAILURE);
                  }
               }
            ...
            // This sure is one of the if() statements of all time. I feel like it can be reduced...
            if ((Cmd->TranslationCtrlActive && Cmd->AttitudeCtrlActive) &&
                     ((!strcmp(Cmd->trn_actuator,"THR_3DOF")&&(!strcmp(Cmd->att_actuator,"THR_6DOF")||!strcmp(Cmd->dmp_actuator,"THR_6DOF"))) ||
                      (!strcmp(Cmd->trn_actuator,"THR_6DOF")&&(!strcmp(Cmd->att_actuator,"THR_3DOF")||!strcmp(Cmd->dmp_actuator,"THR_3DOF"))) ||
                      (!strcmp(Cmd->trn_actuator,"THR_3DOF")&&(!strcmp(Cmd->att_actuator,"THR_3DOF")||!strcmp(Cmd->dmp_actuator,"THR_3DOF"))))) {
               printf("If the Translation actuator is 6DOF Thruster and Attitude actuator is Thruster, then it must be 6DOF (and vice versa).\nAdditionally, if the translation actuator is 3DOF thruster, then Attitude cannot also be 3DOF. Exiting...\n");
               exit(EXIT_FAILURE);
            }
            break;
         }
         ...
         printf("Reached End_Of_File without finding 'DSM_Cmd\tSC[%li]\tCmdTime\t%e'. Exiting...\n",S->ID,DSM->CmdNextTime);
         exit(EXIT_FAILURE);
      }
      else {
         printf("%s from Inp_DSM.txt is not being interpreted. Exiting...\n", DSM_FileLine);
...
void ActuatorModule(struct SCType *S) {
   ...
   // Zero out all actuators first, so that they will be set nonzero only if desired
   for(i = 0; i < 3; i++) AC->IdealFrc[i] = 0.0;
   if (AC->Nthr>0) {
      for(i=0; i<AC->Nthr; i++) AC->Thr[i].PulseWidthCmd  = 0.0;
      for(i=0; i<AC->Nthr; i++) AC->Thr[i].ThrustLevelCmd = 0.0;
   }

   for(i = 0; i < 3; i++) AC->IdealTrq[i] = 0.0;
   if (AC->Nwhl>0)
      for(i=0; i<AC->Nwhl; i++) AC->Whl[i].Tcmd = 0.0;
   if (AC->Nmtb>0)
      for(i=0; i<AC->Nmtb; i++) AC->MTB[i].Mcmd = 0.0;

   for(i = 0; i < 3; i++) AC->Fcmd[i] = 0.0;
   for(i = 0; i < 3; i++) AC->Tcmd[i] = 0.0;
   for(i = 0; i < 3; i++) AC->Mcmd[i] = 0.0;

   // Translation
   if (Cmd->TranslationCtrlActive == TRUE) {
      if ((!strcmp(Cmd->trn_actuator, "THR_3DOF")||!strcmp(Cmd->trn_actuator, "THR_6DOF")) && AC->Nthr>0) {
         for(i = 0; i < 3; i++) AC->Fcmd[i] = DSM->FcmdB[i];
         ThrProcessingMinPower(S);
      }
      else if (!strcmp(Cmd->trn_actuator, "Ideal")) {
         for(i = 0; i < 3; i++) AC->IdealFrc[i] = DSM->FcmdB[i];
      }
      else {}// What is Cmd->trn_actuator if no valid Cmd->ActuatorMode?? Error????
   }

   // Attitude
   if (Cmd->AttitudeCtrlActive == TRUE) {
      if ((!strcmp(Cmd->att_actuator, "THR_3DOF")||!strcmp(Cmd->att_actuator, "THR_6DOF")) && AC->Nthr>0) {
         for(i = 0; i < 3; i++) AC->Tcmd[i] = DSM->Tcmd[i];
         ThrProcessingMinPower(S); // if THR_TRN, this does both force & torque since AC->Fcmd set
      }
      ...
      else {} // What is Cmd->att_actuator if no valid Cmd->AttActuatorMode?? Error????
   }

   // Momentum Dumping
   if (Cmd->H_DumpActive == TRUE) {
      if (Cmd->AttitudeCtrlActive == TRUE && strcmp(Cmd->att_actuator, "WHL")) {
         printf("You many only enable momentum dumping in a PASSIVE attitude state or the attitude is controlled with WHLs. Exiting...\n");
         exit(EXIT_FAILURE);
      }
      if (DSM->DsmCtrl.H_DumpActive == TRUE && !strcmp(Cmd->dmp_actuator, "MTB") && AC->Nmtb>0) {
         CopyUnitV(AC->bvb, unit_bvb);
         ...
         // this could run into issues if Thruster is being used for translation
         for(i = 0; i < 3; i++) AC->Tcmd[i] = DSM->dTcmd[i];
         ThrProcessingMinPower(S); // if THR_TRN, this does both force & torque since AC->Fcmd set
      }
      else if (DSM->DsmCtrl.H_DumpActive == TRUE && !strcmp(Cmd->dmp_actuator, "Ideal")) {
         for(i = 0; i < 3; i++) AC->IdealTrq[i] = DSM->dTcmd[i];
      }
   }

   // Process ActuatorCmd
   for (i = 0; i<Cmd->ActNumCmds; i++) // loops through stored Actuator commands
   {
      if (Cmd->ActTypes[i] == WHL_TYPE) {
         AC->Whl[Cmd->ActInds[i]].Tcmd = AC->Whl[i].Tmax*Cmd->ActDuties[i]/100;
      }
      else if (Cmd->ActTypes[i] == THR_TYPE) {
         AC->Thr[Cmd->ActInds[i]].PulseWidthCmd = Cmd->ActDuties[i]/100*AC->DT;
         AC->Thr[Cmd->ActInds[i]].ThrustLevelCmd = Cmd->ActDuties[i]/100;
      }
      else if (Cmd->ActTypes[i] == MTB_TYPE) {
         AC->MTB[Cmd->ActInds[i]].Mcmd = AC->MTB[i].Mmax*Cmd->ActDuties[i]/100;
      }
   }
}
//------------------------------------------------------------------------------
...
void TranslationGuidance(struct SCType *S) {
   ...
   if (Cmd->TranslationCtrlActive == TRUE && Cmd->ManeuverMode==INACTIVE) {
      ...
      if (!strcmp(Cmd->RefFrame, "F")) {
         ...
         if (F->FixedInFrame == 'L') {
            for(i = 0; i < 3; i++) wcn[i] = Orb[S->RefOrb].wln[i]; // L rotates wrt R
            goodOriginFrame = TRUE;
         }
         else if (F->FixedInFrame == 'N') {
            for(i = 0; i < 3; i++) wcn[i] = 0.0; // R does not rotate wrt R Inertial
            goodOriginFrame = TRUE;
         }
      }
      else if (!strcmp(Cmd->RefFrame, "N")) {
         for(i = 0; i < 3; i++) CTRL->CmdPosR[i] = Cmd->Pos[i]; // Already in R Inertial
         for(i = 0; i < 3; i++) wcn[i] = 0.0; // R does not rotate wrt R Inertial
         goodOriginFrame = TRUE;
      }
      ...
      else if (!strncmp(Cmd->RefFrame, "SC", 2)) { // Specify disp from OP, in SC B frame directions, control to OP
         sscanf(Cmd->RefFrame, "SC[%ld].B[%ld]", &Isc_Ref, &frame_body); // Decode ref SC ID Number
         if (Isc_Ref >= Nsc) {
            printf("This mission only has %ld spacecraft, but spacecraft %ld was attempted to be set as the reference frame. Exiting...\n", Nsc, Isc_Ref);
            exit(EXIT_FAILURE);
         }
         if (frame_body >= SC[Isc_Ref].Nb){
            printf("Spacecraft %ld only has %ld bodies, but the reference frame was attempted to be set as body %ld. Exiting...\n", Isc_Ref, SC[Isc_Ref].Nb, frame_body);
            exit(EXIT_FAILURE);
         }
         QTxV(SC[Isc_Ref].B[frame_body].qn, Cmd->Pos, CTRL->CmdPosR); // Convert SC# B to R Inertial
         QTxV(SC[Isc_Ref].B[frame_body].qn, SC[Isc_Ref].B[frame_body].wn, wcn); // SC rotates wrt R
         goodOriginFrame = TRUE;
      }
      if (!strcmp(Cmd->RefOrigin, "OP")) { // Specify disp from OP, in X frame directions, control to OP
         // Add pos of F frame origin in R frame
         VxV(wcn,CTRL->CmdPosR,CTRL->CmdVelR);
         for (i = 0; i < 3; i++) CTRL->CmdPosR[i] += F->PosR[i];
         goodOriginFrame = TRUE;
      }
      else if (!strncmp(Cmd->RefOrigin, "SC", 2)) { // Specify disp from SC, in X frame directions, control to SC
         // Add pos of SC in R frame
         sscanf(Cmd->RefOrigin, "SC[%ld].B[%ld]", &Isc_Ref, &origin_body); // Decode ref SC ID Number
         if (Isc_Ref >= Nsc) {
            printf("This mission only has %ld spacecraft, but spacecraft %ld was attempted to be set as the reference origin. Exiting...\n", Nsc, Isc_Ref);
            exit(EXIT_FAILURE);
         }
         if (origin_body >= SC[Isc_Ref].Nb){
            printf("Spacecraft %ld only has %ld bodies, but the reference origin was attempted to be set as body %ld. Exiting...\n", Isc_Ref, SC[Isc_Ref].Nb, origin_body);
            exit(EXIT_FAILURE);
         }
         VxV(wcn,CTRL->CmdPosR,CTRL->CmdVelR);
         for (i = 0; i < 3; i++) CTRL->CmdVelR[i] += SC[Isc_Ref].VelR[i] + SC[Isc_Ref].B[origin_body].vn[i];
         for (i = 0; i < 3; i++) CTRL->CmdPosR[i] += SC[Isc_Ref].PosR[i] + SC[Isc_Ref].B[origin_body].pn[i];
         goodOriginFrame = TRUE;
      }
      else {
      ...
      if (goodOriginFrame == FALSE) {
         printf("Invalid Ref origin/frame combo %s/%s in Translation Command at %f. Exiting...\n", Cmd->RefOrigin, Cmd->RefFrame, SimTime);
         exit(EXIT_FAILURE);
      }
      for (i=0; i<3; i++) {
         CTRL->CmdPosN[i] = CTRL->CmdPosR[i] + S->PosN[i]; // Actually calculate these values
   ...
   if (Cmd->AttitudeCtrlActive == TRUE) {
      if (Cmd->Method == PARM_VECTORS) {
         if (PV->TrgType == TARGET_SC || PV->TrgType == TARGET_WORLD) {
            ...
            MxV(S->B[0].CN, PV->N, PriCmdVecB); // (Converting Cmd vec to body frame)
         }
         else if (PV->TrgType == TARGET_VEC) {
            if (!strcmp(Cmd->PriAttRefFrame, "N")) {
               MxV(S->B[0].CN, PV->cmd_vec, PriCmdVecB); // (Converting Cmd vec to body frame)
            }
            else if (!strcmp(Cmd->PriAttRefFrame, "F")) {
               MTxV(F->CN,PV->cmd_vec,PriCmdVecN); // (Converting to Inertial frame)
               MxV(S->B[0].CN, PriCmdVecN, PriCmdVecB); // (Converting to body frame) CHECK THIS
            }
            else if (!strcmp(Cmd->PriAttRefFrame, "L")) {
               MTxV(S->CLN,PV->cmd_vec, PriCmdVecN); // (Converting to LVLH to Inertial frame)
               UNITV(PriCmdVecN);

               MxV(S->B[0].CN, PriCmdVecN, PriCmdVecB); // (Converting to body frame)
            }
            else if (!strcmp(Cmd->PriAttRefFrame, "B")) {
               for(i = 0; i < 3; i++) PriCmdVecB[i] = PV->cmd_vec[i];
            }
         }
         UNITV(PriCmdVecB);
         ...
         if (SV->TrgType == TARGET_SC || SV->TrgType == TARGET_WORLD) {
            ...
            if (!strcmp(Cmd->SecAttRefFrame, "N")) {
               MxV(S->B[0].CN,SV->cmd_vec,SecCmdVecB); // (Converting Cmd vec to body frame)
            }
            else if (!strcmp(Cmd->SecAttRefFrame, "F")) {
               MTxV(F->CN,SV->cmd_vec,SecCmdVecN); // (Converting to Inertial frame)
               MxV(S->B[0].CN,SecCmdVecN,SecCmdVecB); // (Converting to body frame)
            }
            else if (!strcmp(Cmd->SecAttRefFrame, "L")) {
               MTxV(S->CLN,SV->cmd_vec,SecCmdVecN); // (Converting from LVLH to Inertial frame)
               UNITV(SecCmdVecN);
               MxV(S->B[0].CN,SecCmdVecN,SecCmdVecB); // (Converting to body frame)
            }
            else if (!strcmp(Cmd->SecAttRefFrame, "B")) {
               for(i = 0; i < 3; i++) SecCmdVecB[i] = SV->cmd_vec[i];
            }
            UNITV(SecCmdVecB);
         }
         ...
         for (i=0; i<3; i++) vec_cmp[i] = PV->cmd_axis[i] - SV->cmd_axis[i];
         if (fabs(MAGV(vec_cmp)) < EPS_DSM){
            printf("PV [%lf  %lf  %lf] in %s and SV [%lf  %lf  %lf] in %s are identical, resulting in a infeasible attitude command. Exiting...\n", PV->cmd_vec[0], PV->cmd_vec[1], PV->cmd_vec[2], Cmd->PriAttRefFrame, SV->cmd_vec[0], SV->cmd_vec[1], SV->cmd_vec[2], Cmd->SecAttRefFrame);
            exit(EXIT_FAILURE);
         }
         ...
         if (tmag > EPS_DSM) { // check if wmag large enough to avoid singularity
            for (i=0;i<3;i++) Cmd->wrn[i] *= tmag/sin(tmag);
         }
         ...
         printf("Feature for Singular Primary Unit Vector Pointing not currently fully implemented. Exiting...\n");
         exit(EXIT_FAILURE);
      }
      else if (Cmd->Method == PARM_QUATERNION) {
         if (!strcmp(Cmd->AttRefFrame, "N")) {
            for(i = 0; i < 4; i++) Cmd->qrn[i] = Cmd->q[i];
            QxQT(DSM->qbn,Cmd->qrn,Cmd->qbr);
            for (i=0;i<3;i++) Cmd->wrn[i]=0.0;
         }
         else if (!strcmp(Cmd->AttRefFrame, "F")) {
            for(i = 0; i < 4; i++) Cmd->qrf[i] = Cmd->q[i];
            C2Q(F->CN,qfn);
            QxQ(Cmd->qrf,qfn,qrn);
            QxQT(DSM->qbn,qrn,Cmd->qbr);
            if (F->FixedInFrame == 'L') {
               for(i = 0; i < 3; i++) Cmd->wrn[i] = Orb[S->RefOrb].wln[i]; // F rotates wrt N
            }
            else if (F->FixedInFrame == 'N') {
               for(i = 0; i < 3; i++) Cmd->wrn[i] = 0.0; // N does not rotate wrt N Inertial
            }
         }
         else if (!strcmp(Cmd->AttRefFrame, "L")) {
            for(i = 0; i < 4; i++) Cmd->qrl[i] = Cmd->q[i];
            C2Q(F->CL,qfl);
            QxQT(Cmd->qrl,qfl,qrf);
            C2Q(F->CN,qfn);
            QxQ(qrf,qfn,qrn);
            QxQT(DSM->qbn,qrn,Cmd->qbr);
            for (i=0;i<3;i++) Cmd->wrn[i]=Orb[S->RefOrb].wln[i];
         }
      }
      else if (Cmd->Method == PARM_MIRROR) {
         sscanf(Cmd->AttRefScID, "SC[%ld].B[%ld]", &Isc_Ref, &target_num); // Decode ref SC ID Number - does this need to be SC[%ld].B[%ld]?
         if (Isc_Ref >= Nsc) {
            printf("This mission only has %ld spacecraft, but spacecraft %ld was attempted to be set as the spacecraft to mirror. Exiting...\n", Nsc, Isc_Ref);
            exit(EXIT_FAILURE);
         }
         if (target_num >= SC[Isc_Ref].Nb){
            printf("Spacecraft %ld only has %ld bodies, but the mirror target was attempted to be set as body %ld. Exiting...\n", Isc_Ref, SC[Isc_Ref].Nb, target_num);
            exit(EXIT_FAILURE);
         }
         ...
         for(i = 0; i < 3; i++) Cmd->qbr[i] = 0;
         Cmd->qbr[3] = 1;
         for (i=0;i<3;i++) Cmd->wrn[i]=0.0;
      }

      for(i = 0; i < 4; i++) CTRL->qbr[i] = Cmd->qbr[i];
      for(i = 0; i < 3; i++) {
...
void AttitudeNavigation(struct SCType *S) {
   ...
   for(i=0; i<4; i++) DSM->qbn[i] = AC->qbn[i];
   for(i=0; i<3; i++) DSM->wbn[i] = AC->wbn[i];
...
void TranslationCtrl(struct SCType *S) {
   ...
   if (Cmd->TranslationCtrlActive == TRUE && Cmd->ManeuverMode==INACTIVE) {
      // TO DO: do we want S->PosR to be AC->PosR? and DSM->FcmdB to be CTRL->FcmdB??
      if (Cmd->trn_controller == PID_CNTRL) { // PID Controller
         ...
         for(i = 0; i < 3; i++) {
            ...
            DSM->trn_ei[i] += (DSM->Oldperr[i]+DSM->perr[i])/2.0*AC->DT; // Integrated Error

            if (fabs(Cmd->trn_kilimit[i]) > EPS_DSM && fabs(CTRL->trn_ki[i]) > EPS_DSM) DSM->trn_ei[i] = Limit(DSM->trn_ei[i], -Cmd->trn_kilimit[i]/CTRL->trn_ki[i], Cmd->trn_kilimit[i]/CTRL->trn_ki[i]); // limits integrated error to limit/ki, since limit is given in terms of force

            CTRL->u1[i] = CTRL->trn_kp[i]/CTRL->trn_kr[i]*DSM->perr[i];
            if (CTRL->vel_max[i] > 0) {
               CTRL->u1[i] = Limit(CTRL->u1[i], -CTRL->vel_max[i], CTRL->vel_max[i]);
            }
            CTRL->FcmdN[i] = -CTRL->trn_kr[i]*(CTRL->u1[i] + DSM->verr[i]) - CTRL->trn_ki[i]*DSM->trn_ei[i];
         }
         MxV(S->B[0].CN, CTRL->FcmdN, CTRL->FcmdB); // Converting from Inertial to body frame for Report
         for(i = 0; i < 3; i++) {
            if (CTRL->FrcB_max[i] > 0) CTRL->FcmdB[i] = Limit(CTRL->FcmdB[i], -CTRL->FrcB_max[i], CTRL->FrcB_max[i]); // Limiting AC->Frc in body frame
         }
         ...
         for(i = 0; i < 3; i++) {
            DSM->perr[i] = S->PosR[i] - CTRL->CmdPosR[i]; // Position Error, Relative
            DSM->verr[i] = S->VelR[i] - CTRL->CmdVelR[i]; // Velocity Error
         }
         ...
         double dg[3];
         for (i=0; i<3; i++) dg[i] = -mu/pow(r_norm, 3)*S->PosN[i] + mu/pow(r_cntrl, 3)*CTRL->CmdPosN[i];

         for (i = 0; i < 3; i++) {
            CTRL->FcmdN[i] = -CTRL->trn_kp[i]*DSM->perr[i] -CTRL->trn_kr[i]*DSM->verr[i] - dg[i]*AC->mass;
         }
         MxV(S->B[0].CN, CTRL->FcmdN, CTRL->FcmdB); // Converting from Inertial to body frame for Report
         for(i = 0; i < 3; i++) {
            if (CTRL->FrcB_max[i] > 0) CTRL->FcmdB[i] = Limit(CTRL->FcmdB[i], -CTRL->FrcB_max[i], CTRL->FrcB_max[i]); // Limiting AC->Frc in body frame
      ...
      for (i=0; i<3; i++) DSM->Oldperr[i] = DSM->perr[i];
   }
   else if (Cmd->TranslationCtrlActive == TRUE && Cmd->ManeuverMode != INACTIVE) {
         ...
         if (Cmd->ManeuverMode == CONSTANT)
         {
            ...
            for(i = 0; i < 3; i++) {
               if (CTRL->FrcB_max[i] > 0) {
                  CTRL->FcmdB[i] = Limit(CTRL->FcmdB[i], -CTRL->FrcB_max[i], CTRL->FrcB_max[i]); // Limiting AC->Frc in body frame
               }
            }
            MTxV(S->B[0].CN, CTRL->FcmdB, CTRL->FcmdN); // Converting back to Inertial from body frame
         }
         else if (Cmd->ManeuverMode == SMOOTHED) {
            double sharp = -2*atanh(-.99998)/Cmd->BurnTime; // .99998 corresponds to capturing 99.999% of the burn since tanh has an asymptote
            double t_mid = Cmd->BurnStopTime - Cmd->BurnTime/2.0;
            double t_since_mid = SimTime - t_mid ; // Time elapsed since middle of burn

            if (!strcmp(Cmd->RefFrame, "N")) {
               for(i = 0; i < 3; i++) {
                  CTRL->FcmdN[i] = S->mass*(Cmd->DeltaV[i]*sharp/2.0)/((cosh(sharp*t_since_mid))*(cosh(sharp*t_since_mid)));
               }
               MxV(S->B[0].CN, CTRL->FcmdN, CTRL->FcmdB); // Converting from Inertial to body frame for Report
            }
            else if (!strcmp(Cmd->RefFrame, "B")) {
               for(i = 0; i < 3; i++) {
                  CTRL->FcmdB[i] = S->mass*(Cmd->DeltaV[i]*sharp/2.0)/((cosh(sharp*t_since_mid))*(cosh(sharp*t_since_mid)));
               }
            }
            for(i = 0; i < 3; i++) {
               if (CTRL->FrcB_max[i] > 0) {
...
void AttitudeCtrl(struct SCType *S) {
   ...
   if (Cmd->AttitudeCtrlActive == TRUE) {
      if (Cmd->att_controller == PID_CNTRL) // PID Controller
      {
         ...
         QxV(DSM->qbn,Cmd->wrn,wrb); // Rotate angular velocity into Body frame
         for(i = 0; i < 3; i++) {
            DSM->werr[i] = DSM->wbn[i] - wrb[i]; // Angular Velocity Error (in body frame)
            DSM->att_ei[i] += (DSM->Oldtherr[i]+DSM->therr[i])/2.0*AC->DT; // Integrated angle error

            if (fabs(Cmd->att_kilimit[i]) > EPS_DSM && CTRL->att_ki[i] > EPS_DSM) DSM->att_ei[i] = Limit(DSM->att_ei[i], -Cmd->att_kilimit[i]/CTRL->att_ki[i], Cmd->att_kilimit[i]/CTRL->att_ki[i]);

            CTRL->u2[i] = CTRL->att_kp[i]/CTRL->att_kr[i]*DSM->therr[i];
            if (CTRL->w_max[i] > 0) {
               CTRL->u2[i] = Limit(CTRL->u2[i], -CTRL->w_max[i], CTRL->w_max[i]);
            }
            CTRL->Tcmd[i] = -CTRL->att_kr[i]*(CTRL->u2[i] + DSM->werr[i]) - CTRL->att_ki[i]*DSM->att_ei[i];
         }
         for (i=0; i<3; i++) DSM->Oldtherr[i] = DSM->therr[i];
      }
      else if (Cmd->att_controller == LYA_ATT_CNTRL)
      {
         ...
         Q2AngleVec(CTRL->qbr,DSM->therr); // Angular Position Error
         QxV(DSM->qbn,Cmd->wrn,wrb); // Rotate angular velocity into Body frame
         for (i = 0; i < 3; i++) DSM->werr[i] = DSM->wbn[i] - wrb[i]; // Angular Velocity Error (in body frame)

         vxMov(DSM->werr, AC->MOI, om_x_I_om); // calculate nonlinear term in Quaternion Lyapunov stability

         for(i = 0; i<3; i++) {
            CTRL->Tcmd[i] = -Cmd->att_kp[i]*CTRL->qbr[i] - Cmd->att_kr[i]*DSM->werr[i] + om_x_I_om[i];
         }
      }
      for (i=0; i<3; i++) {
         if (CTRL->Trq_max[i]>0)
            CTRL->Tcmd[i] = Limit(CTRL->Tcmd[i], -CTRL->Trq_max[i], CTRL->Trq_max[i]);
      }
   }
   else {
      for(i = 0; i < 3; i++) CTRL->Tcmd[i] = 0;
   }
   // Assigning CMDs to upper structure
   for(i = 0; i < 3; i++) {
...
void MomentumDumpCtrl(struct SCType *S) {
   ...
   for (i = 0; i < 3; i++) whlHNorm += TotalWhlH[i]*TotalWhlH[i];
   whlHNorm = sqrt(whlHNorm);

   if (Cmd->H_DumpActive==FALSE || ((CTRL->H_DumpActive == TRUE) && (whlHNorm<Cmd->H_DumpLims[0])))
      CTRL->H_DumpActive = FALSE;
   else if (Cmd->H_DumpActive==TRUE && ((CTRL->H_DumpActive == FALSE) && (whlHNorm>Cmd->H_DumpLims[1])))
      CTRL->H_DumpActive = TRUE;

   if (CTRL->H_DumpActive == TRUE) {
      switch (Cmd->dmp_controller) {
         case H_DUMP_CNTRL:
            for(i = 0; i < 3; i++) CTRL->dTcmd[i] = -CTRL->dmp_kp[i]*TotalWhlH[i];
            break;
      }

      for (i=0; i<3; i++) {
         if (CTRL->dTrq_max[i]>0)
            CTRL->dTcmd[i] = Limit(CTRL->dTcmd[i], -CTRL->dTrq_max[i], CTRL->dTrq_max[i]);
      }
   }
   else {
      for(i = 0; i < 3; i++) CTRL->dTcmd[i] = 0.0;
   }

   for(i = 0; i < 3; i++) DSM->dTcmd[i] = CTRL->dTcmd[i];
...
void DsmFSW(struct SCType *S) {
   ...
   if (DSM->CmdInit) {
      ...
      for (int i=0; i<3; i++) { // put place holders in integrator "old" values, set ei values to zero to initialize integrated error
         DSM->Oldtherr[i] = 0.0;
         DSM->Oldperr[i] = 0.0;
