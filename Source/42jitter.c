...
void ShakerJitter(struct ShakerType *Sh,
                  struct SCType *S __attribute__((unused)))
{
   ...
   for (It = 0; It < Sh->Ntone; It++) {
      Sh->Output +=
          Sh->ToneAmp[It] * cos(Sh->ToneFreq[It] * SimTime + Sh->TonePhase[It]);
   }
...
void WheelJitter(struct WhlType *W, struct SCType *S __attribute__((unused)))
{
   ...
   for (Ih = 0; Ih < W->NumHarm; Ih++) {
      ...
      if (nOM < 0.1 * TwoPi / DTSIM) {
         ...
         /* Radial Torque */
         CosTerm =
             W->RockFreq * W->RockFreq + (2.0 * W->gamma - H->n) * H->n * OM2;
