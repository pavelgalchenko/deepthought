              ...
              double ApRad, double IntPnt[3], double OutDir[3])
{
   ...
   while (fabs(dl) > eps && k < 10) {
      for (i = 0; i < 3; i++)
         IntPnt[i] = FldPnt[i] + l * FldDir[i];
      for (i = 0; i < 3; i++)
         cp[i] = IntPnt[i] - ctr[i];
      PoA = VoV(cp, axis);
      for (i = 0; i < 3; i++)
         rvec[i] = cp[i] - PoA * axis[i];
...
/**********************************************************************/
long PlanarMirror(double FldPnt[3], double FldDir[3], double ctr[3],
                  double axis[3], double ApRad, double IntPnt[3],
                  double ReflDir[3])
{
   double eps = 1.0E-12;
   double dl  = 1.0E6;
   ...
   LoN = VoV(FldDir, nhat);
   for (i = 0; i < 3; i++)
      ReflDir[i] = FldDir[i] - 2.0 * LoN * nhat[i];

   return (InAperture);
}
/**********************************************************************/
long ConicMirror(double FldPnt[3], double FldDir[3], double ctr[3],
                 double axis[3], double foclen, double ConicConst,
                 double ConicSign, double ApRad, double IntPnt[3],
                 double ReflDir[3])
{
   double eps = 1.0E-12;
   double dl  = 1.0E6;
   ...
   for (i = 0; i < 3; i++)
      nhat[i] = (-ConicSign * rvec[i] + D * axis[i]) / Den;
   LoN = VoV(FldDir, nhat);
   for (i = 0; i < 3; i++)
      ReflDir[i] = FldDir[i] - 2.0 * LoN * nhat[i];
                  ...
                  double OutDirB[3])
{
   ...
   for (Io = 0; Io < Nopt; Io++) {
      ...
      printf("Opt[%ld]:  InPnt: %lf %lf %lf   InDirB: %lf %lf %lf\n", Io,
             InPntB[0], InPntB[1], InPntB[2], InDirB[0], InDirB[1], InDirB[2]);
      /* Find Reflection */
      switch (O->Type) {
         case OPT_APERTURE:
